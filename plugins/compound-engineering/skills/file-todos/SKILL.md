---
name: file-todos
description: This skill should be used when managing the file-based todo tracking system in the configured todo directory. It provides workflows for creating todos, managing status and dependencies, conducting triage, and integrating with slash commands and code review processes.
...
The `$TODOS_DIR/` directory contains a file-based tracking system for managing code review feedback, technical debt, feature requests, and work items. Each todo is a markdown file with YAML frontmatter and structured sections.

Before any file operation, resolve `TODOS_DIR` from `compound-engineering.local.md` frontmatter:
1. `artifact_paths.todos`
2. `artifact_paths.root + "/todos"`
3. default: `todos`
